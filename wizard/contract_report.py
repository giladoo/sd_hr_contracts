...
class SdHrContractDuplacate(models.TransientModel):
    ...
    def all_employees_contract(self):
        ...
        contract_url = f"{base_url}/web/login?redirect=/web#model=hr.contract&view_type=form&id="
        employee_url = f"{base_url}/web/login?redirect=/web#model=hr.employee&view_type=form&id="
        ...
        for row_idx, row in enumerate(report_1):
            for col_idx, value in enumerate(row):
                if row_idx:
                    if col_idx == EMP_COL:
                        # sheet.write_url(row_idx, col_idx, f"{base_url}/web/login?redirect=/web#id={value[0]}&model=hr.employee&view_type=form", link_style, string=value[1])
                        sheet.write_url(row_idx, col_idx, f"{employee_url}{value[0]}", link_style, string=value[1])
                    elif col_idx == CON_COL and value[0]:
                        sheet.write_url(row_idx, col_idx, f"{contract_url}{value[0]}", link_style, string=value[1])
